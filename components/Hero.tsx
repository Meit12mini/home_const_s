...
const BenefitCard = ({ icon, title, description, onClick }: { icon: React.ReactNode; title: string; description: string; onClick: () => void; }) => (
    <button onClick={onClick} className="bg-black/40 backdrop-blur-sm p-6 rounded-xl text-center bg-black/60 transition-all duration-300 transform hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-brand-gold focus:ring-offset-4 focus:ring-offset-brand-dark w-full">
        {icon}
